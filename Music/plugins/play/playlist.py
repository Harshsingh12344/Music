...
from Music import Carbon, YouTube, app
from Music.utils.database import (delete_playlist, get_playlist,
                                       get_playlist_names,
                                       save_playlist)
...
from Music.utils.inline.playlist import (botplaylist_markup,
                                              get_playlist_markup,
                                              warning_markup)
from Music.utils.pastebin import Musicbin
...
async def check_playlist(client, message: Message, _):
    ...
    msg = _["playlist_4"]
    count = 0
    for mukesh in _playlist:
        _note = await get_playlist(message.from_user.id, mukesh)
        title = _note["title"]
        title = title.title()
